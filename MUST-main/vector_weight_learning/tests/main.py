...
DATASET_NAME = os.environ.get("DATASET_NAME", "uqv")
EPOCHS = int(os.environ.get("EPOCHS", 5 if DATASET_NAME == "uqv" else 40))

base_modal1, base_modal2, base_modal3, base_modal4, query_modal1, query_modal2, query_modal3, query_modal4, \
gt = dataset.DatasetLoader(DATASET_NAME)
# gt = [gt1[i] for i in range(10000)]

if DATASET_NAME == "uqv" and (len(base_modal2) == 0 or len(query_modal2) == 0):
    k = int(os.environ.get("TOPK", 1))
    print("UQV modal2 not found; falling back to single-modal evaluation.")
    print("Set UQV_MODAL2_BASE/UQV_MODAL2_QUERY or provide uqv_modal2_base/query under the uqv folder.")
    print("Running single-modal evaluation for uqv dataset.")
    dist_opt = pymswl.OptSet(pymswl.Metric.IP_Float, 1, 1)
    dist_opt.load_base_float(base_modal1)
    dist_opt.load_query_float(query_modal1)
    dist_list = dist_opt.float_gen_dist(0, len(query_modal1) - 1)
    topk = max(1, min(k, len(base_modal1)))
    res = []
    for d in dist_list:
        d = np.asarray(d)
    ...
    print(f"Recall@{topk}: {eva_recall(res, gt, topk):.6f}")
    sys.exit(0)

if len(base_modal3) == 0 and len(base_modal1) > 0:
    base_modal3 = np.zeros((len(base_modal1), 1), dtype='float32')
if len(query_modal3) == 0 and len(query_modal1) > 0:
    query_modal3 = np.zeros((len(query_modal1), 1), dtype='float32')
if len(base_modal4) == 0 and len(base_modal1) > 0:
    base_modal4 = np.zeros((len(base_modal1), 1), dtype='float32')
if len(query_modal4) == 0 and len(query_modal1) > 0:
    query_modal4 = np.zeros((len(query_modal1), 1), dtype='float32')

missing = [
    name for name, data in [
        ("base_modal2", base_modal2),
        ("query_modal2", query_modal2),
        ("base_modal3", base_modal3),
        ("query_modal3", query_modal3),
        ("base_modal4", base_modal4),
        ("query_modal4", query_modal4),
    ] if len(data) == 0
]
if missing:
    print(f"Dataset '{DATASET_NAME}' is missing modalities: {', '.join(missing)}")
    sys.exit(1)

def _slice_rows(arr, max_len):
...
max_base = int(os.environ.get("MAX_BASE", 0))
max_query = int(os.environ.get("MAX_QUERY", 0))
max_gt = int(os.environ.get("MAX_GT", 0))

if max_base > 0:
    base_modal1 = _slice_rows(base_modal1, max_base)
    base_modal2 = _slice_rows(base_modal2, max_base)
    base_modal3 = _slice_rows(base_modal3, max_base)
    base_modal4 = _slice_rows(base_modal4, max_base)
    gt = [[idx for idx in row if idx < max_base] for row in gt]
    gt = [row if row else [0] for row in gt]

if max_query > 0:
    query_modal1 = _slice_rows(query_modal1, max_query)
    query_modal2 = _slice_rows(query_modal2, max_query)
    query_modal3 = _slice_rows(query_modal3, max_query)
    query_modal4 = _slice_rows(query_modal4, max_query)
    gt = gt[:len(query_modal1)]

if max_gt > 0:
    gt = [row[:max_gt] for row in gt]
    gt = [row if row else [0] for row in gt]
...
thread_num = int(os.environ.get("THREADS", 64))
use_modal34 = os.environ.get("USE_MODAL34", "1") != "0"

modal2_is_int = np.issubdtype(np.asarray(base_modal2).dtype, np.integer)
if modal2_is_int:
    dist_opt = pymswl.OptSet(pymswl.Metric.IP_Float_AS_Int_Skip0, thread_num, 1)
    if use_modal34:
        ...
        dist_opt.load_1float_2int(base_modal1, query_modal1, base_modal2, query_modal2)
else:
    dist_opt = pymswl.OptSet(pymswl.Metric.IP_Float, thread_num, 1)
    dist_opt.load_1float_2float(base_modal1, query_modal1, base_modal2, query_modal2)
...
if modal2_is_int:
    ...
    if use_modal34:
        ...
        gt_modal3_dist = [np.zeros(len(gt[i]), dtype='float32') for i in range(len(gt))]
        gt_modal4_dist = [np.zeros(len(gt[i]), dtype='float32') for i in range(len(gt))]
else:
    gt_modal2_dist = [dist_opt.modal2_float_dist_by_id(i, gt[i]) for i in range(len(gt))]
    gt_modal3_dist = [np.zeros(len(gt[i]), dtype='float32') for i in range(len(gt))]
    gt_modal4_dist = [np.zeros(len(gt[i]), dtype='float32') for i in range(len(gt))]
...
for epoch in range(EPOCHS):
    ...
    if modal2_is_int:
        ...
        reca = dist_opt.float_float_topk_id(0, len(gt) - 1, gt_len)
    ...
    if modal2_is_int:
        ...
        if use_modal34:
            ...
            reca_modal3_dist = [np.zeros(len(reca[i]), dtype='float32') for i in range(len(reca))]
            reca_modal4_dist = [np.zeros(len(reca[i]), dtype='float32') for i in range(len(reca))]
    else:
        reca_modal2_dist = [dist_opt.modal2_float_dist_by_id(i, reca[i]) for i in range(len(reca))]
        reca_modal3_dist = [np.zeros(len(reca[i]), dtype='float32') for i in range(len(reca))]
        reca_modal4_dist = [np.zeros(len(reca[i]), dtype='float32') for i in range(len(reca))]
